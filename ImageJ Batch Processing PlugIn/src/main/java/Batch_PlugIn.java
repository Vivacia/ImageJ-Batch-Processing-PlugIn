...
public class Batch_PlugIn implements PlugInFilter {
    ...
    private static void showDialog() {
		...
		String[] items = new String[]{"Change Contrast/Brightness", "Greyscale 8-bit", "Crop", "log 10"};
	...
	public static void main(String[] args) throws Exception {
        ...
        int[] pixelDifference = new int[3];
        int[] previousPixel = new int[3];
        int[] afterPixel = new int[3];
        ...
        for (String imageName : images) {
            ...
            if (function.equals("Change Contrast/Brightness")) {
//                if (firstImage) {
//                    previousPixel = image.getPixel(image.getWidth()/2, image.getHeight()/2);
//                    IJ.run(image, "Brightness/Contrast...", "");
//                    IJ.saveAsTiff(image, addressToSaveAt);
//                    image.close();
//                    ImagePlus tempImage = IJ.openImage(addressToSaveAt);
//                    afterPixel = tempImage.getPixel(image.getWidth()/2, image.getHeight()/2);
//                    firstImage = false;
//                    image.close();
//                    for (int i = 0; i < 3; i++) {
//                        IJ.saveAsTiff(image, addressToSaveAt);
//                        image.close();
//                        image = IJ.openImage(addressToSaveAt);
//                        pixelDifference[i] = afterPixel[i] - previousPixel[i];
//                    }
//                } else {
...
//                }
                IJ.run(image, "Brightness/Contrast...", "");
                IJ.saveAsTiff(image, addressToSaveAt);
            } else if (function.equals("Greyscale 8-bit")) {
                IJ.run(image, "8-bit", "");
                IJ.saveAsTiff(image, addressToSaveAt);
                image.close();
